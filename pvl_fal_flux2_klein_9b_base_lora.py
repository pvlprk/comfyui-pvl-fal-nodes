...
class PVL_fal_Flux2Klein9BBaseLora_API:
    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
    ...
    def generate_image(
        ...
        **kwargs,
        ...
        def action(attempt, total_attempts):
            if debug_log:
                print(
                    f"[Flux.2 Klein 9B Base LoRA] attempt {attempt}/{total_attempts} "
            ...
            if loras:
                arguments["loras"] = loras[:3]

            if debug_log:
                print(
                    f"[Flux.2 Klein 9B Base LoRA] payload keys={list(arguments.keys())} "
                    f"loras={len(loras)}"
                )

            model_id = "fal-ai/flux-2/klein/9b/base/lora"
        ...
        try:
            img_tensor = ApiHandler.run_with_retries(
                ...
                on_retry=lambda attempt, total_attempts, e: print(
                    f"[Flux.2 Klein 9B Base LoRA ERROR] attempt {attempt}/{total_attempts} -> {e}"
                ),
            )
            return (img_tensor,)
        except Exception as e:
            print(f"Error generating image with Flux.2 Klein 9B Base LoRA: {str(e)}")
            return ApiHandler.handle_image_generation_error("Flux.2 Klein 9B Base LoRA", e, width=width, height=height)
