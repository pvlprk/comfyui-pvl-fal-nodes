...
class ImageUtils:
    ...
    def mask_to_image(mask_tensor: torch.Tensor) -> torch.Tensor:
        ...
        Returns a 2D/3D float tensor in [0..1] suitable for tensor_to_pil().
        """
        ...
        if mask.ndim == 3:
            # If CHW, convert to HWC then take a single channel.
            if mask.shape[0] in (1, 3, 4) and mask.shape[2] not in (1, 3, 4):
